...
public class OwnerFragment extends Fragment implements OwnerInterface, UserInterface {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(user != null && user.getType() == 1){
            if(users != null && users.size() > 0){
                ...
                ownerRecyclerView.setAdapter(new OwnersAdapter(users, userPresenter));
            }else{
                userInterface = this;
    ...
    private void init(){
        if(user != null && user.getType() == 1){
            ownerText.setText("المُلاّك");
            ownerNoImage.setImageResource(R.drawable.empty_owner);
        }
    ...
    public void users(List<User> users) {
        ...
        if(users!= null){
            ownerNoImage.setVisibility(View.GONE);
            ownerRecyclerView.setVisibility(View.VISIBLE);
            OwnersAdapter ownersAdapter = new OwnersAdapter(users, userPresenter);
            ownerRecyclerView.setAdapter(ownersAdapter);
            ownerRecyclerView.setLayoutManager(new LinearLayoutManager(App.getContext()));
        }else{
            ownerNoImage.setVisibility(View.VISIBLE);
        }
    }

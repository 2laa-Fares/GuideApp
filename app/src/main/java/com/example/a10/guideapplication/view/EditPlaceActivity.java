...
public class EditPlaceActivity extends AppCompatActivity implements PlaceInterface, DoctorInterface {
    ...
    public void isPlaceAdded(Section section) {
        ...
        if (section != null) {
            ...
            intent.putExtra(getString(R.string.placeId), section.getID());
            intent.putExtra(getString(R.string.placeType), 1);
    ...
    public void doctor(Doctor doctor) {
        ...
        if (doctor != null) {
            Intent intent = new Intent(this, AddBranchActivity.class);
            intent.putExtra(getString(R.string.placeId), doctor.getID());
            intent.putExtra(getString(R.string.placeType), 2);
            startActivity(intent);
            Toast.makeText(this, "لقد تم أضافة البيانات بنجاح", Toast.LENGTH_LONG).show();
            back();
        } else {

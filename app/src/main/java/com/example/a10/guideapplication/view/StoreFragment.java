...
public class StoreFragment extends Fragment implements PlaceInterface{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        RecyclerView.LayoutManager manager2 = new RtlGridLayoutManager(App.getContext(), 3);
        newPlacesRecyclerView.setLayoutManager(manager2);
        if (stores!=null){
            DoctorsAdapter adapter = new DoctorsAdapter(stores, getString(R.string.store));
            ...
            progressBar.show();
            presenter.getPlaces(getString(R.string.store));
        }
        if (topRatedStores != null) {
            DoctorsAdapter adapter2 = new DoctorsAdapter(topRatedStores, getString(R.string.store));
            ...
            presenter.topRated(getString(R.string.store));
        }
        return view;
    }
    ...
    public void places(List<Section> sections) {
        ...
        if (sections != null && sections.size() > 0) {
            this.stores = sections;
            DoctorsAdapter adapter = new DoctorsAdapter(sections, getString(R.string.store));
            newPlacesRecyclerView.setAdapter(adapter);
            newPlacesRecyclerView.setVisibility(View.VISIBLE);
            placesNotFoundTV.setVisibility(View.GONE);
            ...
            placesNotFoundTV.setText("لا يوجد متاجر لعرضها");
        }
    }
    ...
    public void topRated(List sections) {
        if (sections != null && sections.size() > 0) {
            ...
            topRatedNotFoundTV.setText("لا يوجد متاجر لعرضها");
        }
    }
    ...
    public void searchSection(List<Section> section) {
        if (section!=null && section.size()>0){
            ...
            SearchDoctorAdapter adapter = new SearchDoctorAdapter(section, getString(R.string.store));
